...
const performance = import('perf_hooks').performance;
...
function loadCode() {
	return new Promise((resolve, reject) => {
		...
		if (process.env['VSCODE_DEV']) {
			...
			Promise.resolve().then(function () { return require('./bootstrap-node-2ee89e0d.js'); }).injectNodeModuleLookupPath(process.env['VSCODE_INJECT_NODE_MODULE_LOOKUP_PATH']);
		} else {
			delete process.env['VSCODE_INJECT_NODE_MODULE_LOOKUP_PATH'];
		}
		Promise.resolve().then(function () { return require('./bootstrap-amd-074d9f11.js'); }).load('vs/server/node/server.main', resolve, reject);
	});
}
