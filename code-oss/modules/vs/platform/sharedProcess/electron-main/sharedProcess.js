...
let SharedProcess = class SharedProcess extends Disposable {
    ...
    window = undefined;
    windowCloseListener = undefined;
    _onDidError = this._register(new Emitter());
    onDidError = Event.buffer(this._onDidError.event); // buffer until we have a listener!
    constructor(machineId, userEnv, environmentMainService, userDataProfilesService, lifecycleMainService, logService, policyService, themeMainService, protocolMainService) {
        super();
    ...
    registerListeners() {
        ...
        validatedIpcMain.on('vscode:createSharedProcessMessageChannel', (e, nonce) => this.onWindowConnection(e, nonce));
        // Shared process worker relay
        validatedIpcMain.on('vscode:relaySharedProcessWorkerMessageChannel', (e, configuration) => this.onWorkerConnection(e, configuration));
        // Lifecycle
    ...
    _whenReady = undefined;
    whenReady() {
        if (!this._whenReady) {
            // Overall signal that the shared process window was loaded and
            // all services within have been created.
            this._whenReady = new Promise(resolve => validatedIpcMain.once('vscode:shared-process->electron-main=init-done', () => {
    ...
    get whenIpcReady() {
        if (!this._whenIpcReady) {
            this._whenIpcReady = (async () => {
                ...
                await new Promise(resolve => validatedIpcMain.once('vscode:shared-process->electron-main=ipc-ready', () => {
    ...
    registerWindowListeners() {
        ...
        this.window.webContents.on('render-process-gone', (event, details) => this._onDidError.fire({ type: 2 /* WindowError.PROCESS_GONE */, details }));
...
SharedProcess = __decorate([
    ...
    __param(4, ILifecycleMainService),
    __param(5, ILogService),
    __param(6, IPolicyService),
    __param(7, IThemeMainService),
    __param(8, IProtocolMainService)
], SharedProcess);
