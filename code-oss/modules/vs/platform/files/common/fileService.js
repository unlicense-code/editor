...
import { bufferedStreamToBuffer, bufferToReadable, newWriteableBufferStream, readableToBuffer, streamToBuffer, VSBuffer } from 'vs/base/common/buffer';
...
import { extUri, extUriIgnorePathCase, isAbsolutePath } from 'vs/base/common/resources';
...
let FileService = class FileService extends Disposable {
    ...
    async doReadFileStream(provider, resource, options, token) {
        ...
        try {
            ...
            // read unbuffered
            if ((options?.atomic && hasFileAtomicReadCapability(provider)) || // atomic reads are always unbuffered
    ...
    _onDidFilesChange = this._register(new Emitter());
    onDidFilesChange = this._onDidFilesChange.event;
    ...
    throwIfFileSystemIsReadonly(provider, resource) {
        if (provider.capabilities & 2048 /* FileSystemProviderCapabilities.Readonly */) {
            throw new FileOperationError(localize('err.readonly', "Unable to modify readonly file '{0}'", this.resourceForError(resource)), 6 /* FileOperationResult.FILE_PERMISSION_DENIED */);
        }
        return provider;
    }
