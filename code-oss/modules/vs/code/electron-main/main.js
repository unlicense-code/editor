...
import { isValidBasename, parseLineAndColumnAware, sanitizeFilePath } from 'vs/base/common/extpath';
...
import { connect as nodeIPCConnect, serve as nodeIPCServe, XDG_RUNTIME_DIR } from 'vs/base/parts/ipc/node/ipc.net';
...
class CodeMain {
        ...
        try {
            ...
            await instantiationService.invokeFunction(async (accessor) => {
                ...
                once(lifecycleMainService.onWillShutdown)(evt => {
                    ...
                    evt.join(FSPromises.unlink(environmentMainService.mainLockfile).catch(() => { }));
                });
    ...
    async initServices(environmentMainService, userDataProfilesMainService, configurationService, stateMainService, productService) {
        ...
        try {
            ...
            try {
                ...
                if (!retry || isWindows || error.code !== 'ECONNREFUSED') {
                    if (error.code === 'EPERM') {
                        this.showStartupWarningDialog(localize('secondInstanceAdmin', "Another instance of {0} is already running as administrator.", productService.nameShort), localize('secondInstanceAdminDetail', "Please close the other instance and try again."), productService.nameLong);
                    }
                    throw error;
                }
            ...
            if (!environmentMainService.args.wait && !environmentMainService.args.status) {
                startupWarningDialogHandle = setTimeout(() => {
                    this.showStartupWarningDialog(localize('secondInstanceNoResponse', "Another instance of {0} is running but not responding", productService.nameShort), localize('secondInstanceNoResponseDetail', "Please close all other instances and try again."), productService.nameLong);
                }, 10000);
            }
