...
import { app, BrowserWindow, dialog, protocol, session, systemPreferences } from 'electron';
...
import { isLinux, isLinuxSnap, isMacintosh, isWindows, OS } from 'vs/base/common/platform';
...
let CodeApplication = class CodeApplication extends Disposable {
    ...
    constructor(mainProcessNodeIpcServer, userEnv, mainInstantiationService, logService, environmentMainService, lifecycleMainService, configurationService, stateMainService, fileService, productService, userDataProfilesMainService) {
        super();
    ...
    registerListeners() {
        ...
        validatedIpcMain.handle('vscode:writeNlsFile', (event, path, data) => {
        ...
        validatedIpcMain.handle('vscode:readNlsFile', async (event, ...paths) => {
    ...
    async startup() {
        ...
        const eventuallyPhaseScheduler = this._register(new RunOnceScheduler(() => {
            this._register(runWhenIdle(() => this.lifecycleMainService.phase = 4 /* LifecycleMainPhase.Eventually */, 2500));
        }, 2500));
        eventuallyPhaseScheduler.schedule();
    }
    setUpHandlers(instantiationService) {
        // Auth Handler
    ...
    async resolveMachineId() {
        ...
        let machineId = this.stateMainService.getItem(machineIdKey);
    ...
    async openFirstWindow(accessor, mainProcessElectronServer) {
    ...
    afterWindowOpen(accessor, sharedProcess) {
    ...
    handleSharedProcessErrors(telemetryService, sharedProcess) {
...
CodeApplication = __decorate([
    ...
    __param(7, IStateMainService),
    __param(8, IFileService),
    __param(9, IProductService),
    __param(10, IUserDataProfilesMainService)
], CodeApplication);
