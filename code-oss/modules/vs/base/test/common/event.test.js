...
import { AsyncEmitter, DebounceEmitter, Emitter, Event, EventBufferer, EventMultiplexer, MicrotaskEmitter, PauseableEmitter, Relay } from 'vs/base/common/event';
import { DisposableStore, isDisposable, setDisposableTracker, toDisposable } from 'vs/base/common/lifecycle';
...
suite('PausableEmitter', function () {
    ...
    test('pause/resume - merge', function () {
        const data = [];
        const emitter = new PauseableEmitter({ merge: (a) => a.reduce((p, c) => p + c, 0) });
        emitter.event(e => data.push(e));
        emitter.fire(1);
        emitter.fire(2);
        assert.deepStrictEqual(data, [1, 2]);
