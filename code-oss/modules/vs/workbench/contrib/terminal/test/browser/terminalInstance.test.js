...
export function createInstance(partial) {
...
suite('Workbench - TerminalInstance', () => {
    ...
    suite('TerminalLabelComputer', () => {
        ...
        test('should resolve workspaceFolder', () => {
            ...
            configHelper = new TerminalConfigHelper(configurationService, null, null, null, null);
            terminalLabelComputer = new TerminalLabelComputer(configHelper, createInstance({ capabilities, processName: 'zsh', workspaceFolder: { uri: URI.from({ scheme: Schemas.file, path: 'folder' }) } }), mockContextService);
            terminalLabelComputer.refreshLabel();
        ...
        test('should always return static title when specified', () => {
            ...
            configHelper = new TerminalConfigHelper(configurationService, null, null, null, null);
            terminalLabelComputer = new TerminalLabelComputer(configHelper, createInstance({ capabilities, processName: 'process', workspaceFolder: { uri: URI.from({ scheme: Schemas.file, path: 'folder' }) }, staticTitle: 'my-title' }), mockContextService);
            terminalLabelComputer.refreshLabel();
        ...
        test('should provide cwdFolder for all cwds only when in multi-root', () => {
            ...
            configHelper = new TerminalConfigHelper(configurationService, null, null, null, null);
            terminalLabelComputer = new TerminalLabelComputer(configHelper, createInstance({ capabilities, processName: 'process', workspaceFolder: { uri: URI.from({ scheme: Schemas.file, path: ROOT_1 }) }, cwd: ROOT_1 }), mockContextService);
            terminalLabelComputer.refreshLabel();
            ...
            configurationService = new TestConfigurationService({ terminal: { integrated: { tabs: { separator: ' ~ ', title: '${process}${separator}${cwdFolder}', description: '${cwdFolder}' } } } });
            configHelper = new TerminalConfigHelper(configurationService, null, null, null, null);
            terminalLabelComputer = new TerminalLabelComputer(configHelper, createInstance({ capabilities, processName: 'process', workspaceFolder: { uri: URI.from({ scheme: Schemas.file, path: ROOT_1 }) }, cwd: ROOT_2 }), mockMultiRootContextService);
            terminalLabelComputer.refreshLabel();
            if (isWindows) {
                strictEqual(terminalLabelComputer.title, 'process');
                strictEqual(terminalLabelComputer.description, '');
        ...
        test('should hide cwdFolder in single folder workspaces when cwd matches the workspace\'s default cwd even when slashes differ', async () => {
            ...
            strictEqual(terminalLabelComputer.description, '');
            if (!isWindows) {
                terminalLabelComputer = new TerminalLabelComputer(configHelper, createInstance({ capabilities, processName: 'process', workspaceFolder: { uri: URI.from({ scheme: Schemas.file, path: ROOT_1 }) }, cwd: ROOT_2 }), mockContextService);
                terminalLabelComputer.refreshLabel();
                strictEqual(terminalLabelComputer.title, 'process ~ root2');
                strictEqual(terminalLabelComputer.description, 'root2');
            }
