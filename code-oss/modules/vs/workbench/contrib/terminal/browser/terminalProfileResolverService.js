...
export class BaseTerminalProfileResolverService {
    _context;
    ...
    async showProfileMigrationNotification() {
        ...
        if (shouldMigrateToProfile && this._storageService.getBoolean(SHOULD_PROMPT_FOR_PROFILE_MIGRATION_KEY, 1 /* StorageScope.WORKSPACE */, true) && !migrationMessageShown) {
            this._notificationService.prompt(Severity.Info, localize('terminalProfileMigration', "The terminal is using deprecated shell/shellArgs settings, do you want to migrate it to a profile?"), [
                {
                    ...
                    run: async () => {
                        ...
                        if (typeof profile === 'string') {
                            ...
                            const profiles = { ...this._configurationService.inspect("terminal.integrated.profiles." /* TerminalSettingPrefix.Profiles */ + this._primaryBackendOs).userValue };
