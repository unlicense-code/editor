...
import { IExtensionManagementService, IExtensionGalleryService, getTargetPlatform } from 'vs/platform/extensionManagement/common/extensionManagement';
...
import { IExperimentService, ExperimentActionType, ExperimentService } from 'vs/workbench/contrib/experiments/common/experimentService';
...
suite('ExtensionsViews Tests', () => {
    ...
    setup(async () => {
        ...
        await instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement([localDisabledTheme], 6 /* EnablementState.DisabledGlobally */);
        await instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement([localDisabledLanguage], 6 /* EnablementState.DisabledGlobally */);
        instantiationService.set(IExtensionsWorkbenchService, instantiationService.createInstance(ExtensionsWorkbenchService));
        testableView = instantiationService.createInstance(ExtensionsListView, {}, { id: '', title: '' });
    });
    teardown(() => {
        instantiationService.get(IExtensionsWorkbenchService).dispose();
    ...
    test('Test @recommended query', () => {
        ...
        return testableView.show('@recommended').then(result => {
            const extensionInfos = target.args[1][0];
    ...
    test('Test curated list experiment', () => {
        ...
        const experimentTarget = instantiationService.stubPromise(IExperimentService, 'getCuratedExtensionsList', curatedList.map(e => e.identifier.id));
        const queryTarget = instantiationService.stubPromise(IExtensionGalleryService, 'getExtensions', curatedList);
    ...
    test('Test preferred search experiment', () => {
        ...
        const experimentTarget = instantiationService.stubPromise(IExperimentService, 'getExperimentsByType', [{
                id: 'someId',
                enabled: true,
