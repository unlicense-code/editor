...
import { ExtensionsLocalizedLabel, IExtensionManagementService, IExtensionGalleryService, PreferencesLocalizedLabel } from 'vs/platform/extensionManagement/common/extensionManagement';
import { IExtensionManagementServerService, IWorkbenchExtensionEnablementService, IWorkbenchExtensionManagementService } from 'vs/workbench/services/extensionManagement/common/extensionManagement';
...
// Register Commands
CommandsRegistry.registerCommand('_extensions.manage', (accessor, extensionId, tab) => {
...
CommandsRegistry.registerCommand('extension.open', async (accessor, extensionId, tab) => {
    const extensionService = accessor.get(IExtensionsWorkbenchService);
...
let ExtensionsContributions = class ExtensionsContributions extends Disposable {
    ...
    registerGlobalActions() {
        ...
        this.registerExtensionAction({
            ...
            run: (accessor) => accessor.get(IConfigurationService).updateValue(AutoUpdateConfigurationKey, true)
        });
        this.registerExtensionAction({
            ...
            run: (accessor) => accessor.get(IConfigurationService).updateValue(AutoUpdateConfigurationKey, 'onlyEnabledExtensions')
        });
        this.registerExtensionAction({
            ...
            run: (accessor) => accessor.get(IConfigurationService).updateValue(AutoUpdateConfigurationKey, false)
        });
        this.registerExtensionAction({
            ...
            run: () => {
                return Promise.all(this.extensionsWorkbenchService.outdated.map(async (extension) => {
                    try {
                        ...
                        runAction(this.instantiationService.createInstance(PromptExtensionInstallFailureAction, extension, extension.latestVersion, 3 /* InstallOperation.Update */, undefined, err));
                    }
                }));
            }
        });
            ...
            run: async (accessor, resources) => {
                ...
                await Promises.settled(extensions.map(async (vsix) => await extensionsWorkbenchService.install(vsix)))
                    ...
                    for (const extension of extensions) {
                        ...
                        notificationService.prompt(Severity.Info, message, actions);
                    }
                });
            }
        });
...
workbenchRegistry.registerWorkbenchContribution(StatusUpdater, 3 /* LifecyclePhase.Restored */);
