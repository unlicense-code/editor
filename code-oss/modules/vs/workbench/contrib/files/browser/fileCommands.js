...
import { isWorkspaceToOpen } from 'vs/platform/window/common/window';
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor, resource) => {
        ...
        const viewlet = (await paneCompositeService.openPaneComposite(VIEWLET_ID, 0 /* ViewContainerLocation.Sidebar */, false))?.getViewPaneContainer();
...
CommandsRegistry.registerCommand({
    ...
    handler: (accessor, resource) => {
        ...
        const resources = getMultiSelectedResources(resource, accessor.get(IListService), accessor.get(IEditorService), accessor.get(IExplorerService)).filter(resource => workspace.folders.some(folder => uriIdentityService.extUri.isEqual(folder.uri, resource)) // Need to verify resources are workspaces since multi selection can trigger this command on some non workspace resources
        );
