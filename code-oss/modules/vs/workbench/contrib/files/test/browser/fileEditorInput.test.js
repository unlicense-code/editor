...
import { TextFileOperationError } from 'vs/workbench/services/textfile/common/textfiles';
import { FileOperationError, NotModifiedSinceFileOperationError } from 'vs/platform/files/common/files';
...
suite('Files - FileEditorInput', () => {
    ...
    test('resolve handles too large files', async function () {
        const input = createFileInput(toResource.call(this, '/foo/bar/updatefile.js'));
        accessor.textFileService.setReadStreamErrorOnce(new FileOperationError('error', 7 /* FileOperationResult.FILE_TOO_LARGE */));
        const resolved = await input.resolve();
        assert.ok(resolved);
        resolved.dispose();
    });
    ...
    test('file editor serializer', async function () {
        ...
        const input = createFileInput(toResource.call(this, '/foo/bar/updatefile.js'));
        const disposable = Registry.as(EditorExtensions.EditorFactory).registerEditorSerializer('workbench.editors.files.fileEditorInput', FileEditorInputSerializer);
