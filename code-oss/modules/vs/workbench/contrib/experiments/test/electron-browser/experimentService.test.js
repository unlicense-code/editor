...
import { currentSchemaVersion, ExperimentActionType, ExperimentService, getCurrentActivationRecord } from 'vs/workbench/contrib/experiments/common/experimentService';
...
suite('Experiment Service', () => {
    ...
    test('Activation event experiment with not enough events should be evaluating', () => {
        ...
        instantiationService.stub(IStorageService, 'get', (a, b, c) => {
    ...
    test('Activation event updates', async () => {
        ...
        instantiationService.stub(IStorageService, 'store', (key, value, scope) => {
            if (key.includes('experimentEventRecord')) {
                ...
                assert.strictEqual(scope, -1 /* StorageScope.APPLICATION */);
            }
        });
        testObject = instantiationService.createInstance(TestExperimentService);
    ...
    test('Experiment not matching user setting should be disabled', () => {
        ...
        instantiationService.stub(IConfigurationService, 'getValue', (key) => key === 'neat' ? false : undefined);
        testObject = instantiationService.createInstance(TestExperimentService);
        return testObject.getExperimentById('experiment1').then(result => {
            assert.strictEqual(result.enabled, true);
            assert.strictEqual(result.state, 1 /* ExperimentState.NoRun */);
    ...
    test('Experiment matching user setting should be enabled', () => {
        ...
        instantiationService.stub(IConfigurationService, 'getValue', (key) => key === 'neat' ? true : undefined);
        testObject = instantiationService.createInstance(TestExperimentService);
        return testObject.getExperimentById('experiment1').then(result => {
            assert.strictEqual(result.enabled, true);
            assert.strictEqual(result.state, 2 /* ExperimentState.Run */);
