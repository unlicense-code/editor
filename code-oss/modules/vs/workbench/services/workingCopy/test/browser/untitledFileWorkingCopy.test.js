...
import { newWriteableBufferStream, VSBuffer, streamToBuffer, bufferToStream, readableToBuffer } from 'vs/base/common/buffer';
...
suite('UntitledFileWorkingCopy', () => {
    ...
    function createWorkingCopy(uri = resource, hasAssociatedFilePath = false, initialValue = '') {
        return new UntitledFileWorkingCopy('testUntitledWorkingCopyType', uri, basename(uri), hasAssociatedFilePath, initialValue.length > 0 ? { value: bufferToStream(VSBuffer.fromString(initialValue)) } : undefined, factory, async (workingCopy) => { await workingCopy.revert(); return true; }, accessor.workingCopyService, accessor.workingCopyBackupService, accessor.logService);
    }
    setup(() => {
