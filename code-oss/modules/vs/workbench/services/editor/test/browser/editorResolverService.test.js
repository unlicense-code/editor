...
import { createEditorPart, TestFileEditorInput, TestServiceAccessor, workbenchInstantiationService } from 'vs/workbench/test/browser/workbenchTestServices';
    ...
    test('Diff editor Resolve', async () => {
        ...
        const registeredEditor = service.registerEditor('*.test-diff', {
            ...
            createDiffEditorInput: ({ modified, original, options }, group) => ({
                editor: accessor.instantiationService.createInstance(DiffEditorInput, 'name', 'description', new TestFileEditorInput(URI.parse(original.toString()), TEST_EDITOR_INPUT_ID), new TestFileEditorInput(URI.parse(modified.toString()), TEST_EDITOR_INPUT_ID), undefined)
            })
        });
        const resultingResolution = await service.resolveEditor({
