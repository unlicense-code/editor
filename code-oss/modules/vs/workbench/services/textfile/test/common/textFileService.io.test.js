...
import { snapshotToString, stringToSnapshot } from 'vs/workbench/services/textfile/common/textfiles';
...
export default function createSuite(params) {
    ...
    async function testEncoding(resource, encoding, content, expectedContent) {
        ...
        assert.strictEqual(snapshotToString(resolved.value.create(isWindows ? 2 /* DefaultEndOfLine.CRLF */ : 1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false)), expectedContent);
    }
    ...
    async function testEncodingKeepsData(resource, encoding, expected) {
        ...
        const content = snapshotToString(resolved.value.create(isWindows ? 2 /* DefaultEndOfLine.CRLF */ : 1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false));
        assert.strictEqual(content, expected);
        await service.write(resource, content, { encoding });
        resolved = await service.readStream(resource, { encoding });
        assert.strictEqual(snapshotToString(resolved.value.create(2 /* DefaultEndOfLine.CRLF */).textBuffer.createSnapshot(false)), content);
    ...
    async function testReadStream(resource) {
        ...
        const content = (await readFile(resource.fsPath)).toString();
        assert.strictEqual(snapshotToString(result.value.create(1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false)), snapshotToString(createTextModelSnapshot(content, false)));
    }
    ...
    async function testLargeEncoding(encoding, needle) {
        ...
        const result = await service.readStream(resource, { encoding });
        assert.strictEqual(result.encoding, encoding);
        let contents = snapshotToString(result.value.create(1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false));
        ...
        contents = snapshotToString(factory.create(1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false));
        assert.strictEqual(contents.indexOf(needle), 0);
        assert.ok(contents.indexOf(needle, 10) > 0);
    }
