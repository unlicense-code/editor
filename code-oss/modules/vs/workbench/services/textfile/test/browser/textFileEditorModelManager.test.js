...
import { FileChangesEvent, FileOperationError } from 'vs/platform/files/common/files';
...
suite('Files - TextFileEditorModelManager', () => {
    ...
    test('add, remove, clear, get, getAll', function () {
        ...
        const model1 = instantiationService.createInstance(TextFileEditorModel, toResource.call(this, '/path/random1.txt'), 'utf8', undefined);
        const model2 = instantiationService.createInstance(TextFileEditorModel, toResource.call(this, '/path/random2.txt'), 'utf8', undefined);
        const model3 = instantiationService.createInstance(TextFileEditorModel, toResource.call(this, '/path/random3.txt'), 'utf8', undefined);
    ...
    test('resolve (sync) - model disposed when error and first call to resolve', async () => {
        const manager = accessor.textFileService.files;
        const resource = URI.file('/path/index.txt');
        accessor.textFileService.setReadStreamErrorOnce(new FileOperationError('fail', 11 /* FileOperationResult.FILE_OTHER_ERROR */));
        let error = undefined;
        try {
            await manager.resolve(resource);
        }
