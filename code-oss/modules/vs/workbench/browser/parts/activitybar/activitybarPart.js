...
import { ToggleCompositePinnedAction, ActivityAction } from 'vs/workbench/browser/parts/compositeBarActions';
...
let ActivitybarPart = class ActivitybarPart extends Part {
    ...
    static PINNED_VIEW_CONTAINERS = 'workbench.activity.pinnedViewlets2';
    static PLACEHOLDER_VIEW_CONTAINERS = 'workbench.activity.placeholderViewlets';
    static ACTION_HEIGHT = 48;
    static ACCOUNTS_ACTION_INDEX = 0;
    static GEAR_ICON = registerIcon('settings-view-bar-icon', Codicon.settingsGear, localize('settingsViewBarIcon', "Settings icon in the view bar."));
    ...
    globalActivitiesContainer;
    globalActivity = [];
    accountsActivityAction;
    profilesActivityAction;
    accountsActivity = [];
    compositeActions = new Map();
    viewContainerDisposables = new Map();
    keyboardNavigationDisposables = this._register(new DisposableStore());
    location = 0 /* ViewContainerLocation.Sidebar */;
    hasExtensionsRegistered = false;
    enabledViewContainersContextKeys = new Map();
    constructor(paneCompositePart, instantiationService, layoutService, themeService, storageService, extensionService, viewDescriptorService, contextKeyService, configurationService, environmentService, userDataProfileService, userDataProfilesService) {
    ...
    createCompositeBar() {
        ...
        return this._register(this.instantiationService.createInstance(CompositeBar, cachedItems, {
            ...
            dndHandler: new CompositeDragAndDrop(this.viewDescriptorService, 0 /* ViewContainerLocation.Sidebar */, async (id, focus) => { return await this.paneCompositePart.openPaneComposite(id, focus) ?? null; }, (from, to, before) => this.compositeBar.move(from, to, before?.verticallyBefore), () => this.compositeBar.getCompositeBarItems()),
            compositeSize: 52,
        ...
        this.globalActivityActionBar = this._register(new ActionBar(container, {
            actionViewItemProvider: action => {
                ...
                if (action.id === 'workbench.actions.profiles') {
                    return this.instantiationService.createInstance(ProfilesActivityActionViewItem, action, () => this.compositeBar.getContextMenuActions(), (theme) => this.getProfileItemColors(theme), this.getActivityHoverOptions());
                }
    ...
    updateProfilesActivityAction() {
        ...
        if (this.profilesActivityAction) {
            ...
            if (this.profilesActivityAction.activity.icon === activity.icon) {
    ...
    onDidStorageValueChange(e) {
        if (e.key === ActivitybarPart.PINNED_VIEW_CONTAINERS && e.scope === 0 /* StorageScope.PROFILE */
        ...
        if (e.key === AccountsActivityActionViewItem.ACCOUNTS_VISIBILITY_PREFERENCE_KEY && e.scope === 0 /* StorageScope.PROFILE */) {
            this.toggleAccountsActivity();
        }
        if (e.key === ProfilesActivityActionViewItem.PROFILES_VISIBILITY_PREFERENCE_KEY && e.scope === 0 /* StorageScope.PROFILE */) {
            this.toggleProfilesActivityAction();
        }
    }
    ...
    _cachedViewContainers = undefined;
    ...
    getStoredPinnedViewContainersValue() {
        return this.storageService.get(ActivitybarPart.PINNED_VIEW_CONTAINERS, 0 /* StorageScope.PROFILE */, '[]');
    }
    setStoredPinnedViewContainersValue(value) {
        this.storageService.store(ActivitybarPart.PINNED_VIEW_CONTAINERS, value, 0 /* StorageScope.PROFILE */, 0 /* StorageTarget.USER */);
    }
    ...
    getStoredPlaceholderViewContainersValue() {
        return this.storageService.get(ActivitybarPart.PLACEHOLDER_VIEW_CONTAINERS, 0 /* StorageScope.PROFILE */, '[]');
    }
    setStoredPlaceholderViewContainersValue(value) {
        this.storageService.store(ActivitybarPart.PLACEHOLDER_VIEW_CONTAINERS, value, 0 /* StorageScope.PROFILE */, 1 /* StorageTarget.MACHINE */);
    }
    ...
    get profilesVisibilityPreference() {
        return this.userDataProfilesService.isEnabled() && this.storageService.getBoolean(ProfilesActivityActionViewItem.PROFILES_VISIBILITY_PREFERENCE_KEY, 0 /* StorageScope.PROFILE */, this.userDataProfilesService.profiles.length > 1);
    }
    set profilesVisibilityPreference(value) {
        this.storageService.store(ProfilesActivityActionViewItem.PROFILES_VISIBILITY_PREFERENCE_KEY, value, 0 /* StorageScope.PROFILE */, 0 /* StorageTarget.USER */);
    }
    toJSON() {
        return {
