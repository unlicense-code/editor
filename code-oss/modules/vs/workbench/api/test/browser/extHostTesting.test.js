...
suite('ExtHost Testing', () => {
    ...
    suite('OwnedTestCollection', () => {
        ...
        test('treats in-place replacement as mutation', () => {
            ...
            assert.deepStrictEqual(single.collectDiff(), [
                {
                    op: 1 /* TestDiffOpType.Update */,
                    item: { extId: new TestId(['ctrlId', 'id-a']).toString(), expand: 3 /* TestItemExpandState.Expanded */, item: { label: 'Hello world' } },
                },
            ]);
    ...
    suite('TestRunTracker', () => {
        ...
        setup(async () => {
            ...
            configuration = new TestRunProfileImpl(mockObject()(), new Map(), 'ctrlId', 42, 'Do Run', TestRunProfileKind.Run, () => { }, false);
            await single.expand(single.root.id, Infinity);
            single.collectDiff();
            req = {
                include: undefined,
