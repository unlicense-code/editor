...
let EditorWorkerService = class EditorWorkerService extends Disposable {
    ...
    async computeDiff(original, modified, options, algorithm) {
        ...
        const diff = {
            ...
            changes: result.changes.map((c) => new LineRangeMapping(new LineRange(c[0], c[1]), new LineRange(c[2], c[3]), c[4]?.map((c) => new RangeMapping(new Range(c[0], c[1], c[2], c[3]), new Range(c[4], c[5], c[6], c[7]))))),
        };
        return diff;
    }
