...
let DiffReview = class DiffReview extends Disposable {
    _languageService;
    _audioCueService;
    static _ttPolicy = window.trustedTypes?.createPolicy('diffReview', { createHTML: value => value });
    _diffEditor;
    _isVisible;
    shadow;
    _actionBar;
    ...
    _goToRow(row, type) {
        ...
        if (element?.classList.contains("line-insert" /* DiffEditorLineClasses.Insert */)) {
            this._audioCueService.playAudioCue(AudioCue.diffLineInserted, true);
        }
        else if (element?.classList.contains("line-delete" /* DiffEditorLineClasses.Delete */)) {
