...
export function toMenuItems(inputCodeActions, showHeaders) {
    if (!showHeaders) {
    ...
    for (const menuEntry of menuEntries) {
        if (menuEntry.actions.length) {
            ...
            for (const action of menuEntry.actions) {
                allMenuItems.push({ kind: "action" /* ActionListItemKind.Action */, item: action, group: menuEntry.group, label: action.action.title, disabled: !!action.action.disabled });
            }
        }
    }
    return allMenuItems;
