...
suite('standardLinesDiffCompute', () => {
    test('1', () => {
        assert.deepStrictEqual(toJson(lineRangeMappingFromRangeMappings([
...
`.split('\n');
        ...
        assert.deepStrictEqual(toJsonWithDetails(diff.changes), [
            {
                main: "{[5,6)->[5,8)}",
                inner: [
                    "{[5,41 -> 5,41]->[5,41 -> 7,28]}"
...
function toJson(mappings) {
    return mappings.map(m => m.toString());
}
function toJsonWithDetails(mappings) {
